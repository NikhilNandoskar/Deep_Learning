...
def my_model(optimizer):
    ...
    classifier.add(Dense(units = 16, activation = 'relu', kernel_initializer = 'uniform', input_dim = X_test.shape[1] ))
...
tuned_model = KerasClassifier(build_fn=my_model, optimizer='rmsprop', batch_size=32, epochs=100)
tuned_model.fit(X_train, y_train)
result = tuned_model.predict(X_test)

#Evaluate the model
score = tuned_model.evaluate(X_test, y_test)
